<template>
    <div id="index">
      <div id='content'>
        <div class="item" v-for="(item,index) in cardInfo" :key="index" :style="autoHeight">
          <div class="card-title">{{item.title}}</div>
          <div :id='"item-"+index' class="echart-item"></div>
        </div>
      </div>
    </div>
</template>
...
export default {
  ...
  mounted () {
    ...
    this.chartOptionsList.forEach((element, index) => {
      var myChart = echarts.init(document.getElementById('item-' + (index)))
  ...
  computed: {
    autoHeight () {
      console.log(this.cardInfo.length / 2)
      return {
        // 取所有版块/2排列多少行的行数做自适应高度
        height: `calc(96% / ${Math.ceil(this.cardInfo.length / 2)})`
      }
    }
  }
}
